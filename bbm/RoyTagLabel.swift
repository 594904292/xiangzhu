        ...
        for var index = 0; index < textArray?.count; index += 1 {
            ...
            if(!gotPreviousFrame){
                ...
                if(aa  > Float(self.frame.size.width)){
                    newRect.origin = CGPointMake(0, CGFloat(previousFrame.origin.y + textRect.height + Roy_BOTTOM_MARGIN));
                    totalHeight = CGFloat(totalHeight! + textRect.size.height + Roy_BOTTOM_MARGIN);
                }else{
                    newRect.origin = CGPointMake(CGFloat(previousFrame.origin.x + previousFrame.size.width + Roy_LABEL_MARGIN), CGFloat(previousFrame.origin.y));
                }
